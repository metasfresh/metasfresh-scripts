...
check_replication_status(){
 ...
 local sr_standby_sql=$(ssh ${REP_DB_USER}@${REP_DB_HOST} 'psql -t -A -c "select pg_is_in_recovery();"')
...
start_replication(){
 ...
 if [[ "$SR_IS_STANDBY" == "YES" ]]; then
        echo "[OK] ${REP_DB_HOST} is running and connected to primary server!"
 else
 ...
 chmod 770 ${SYNC_SCRIPT}
 sudo /bin/bash ${SYNC_SCRIPT}
...
stop_replication(){
 ...
 if [[ "$SR_IS_STANDBY" == "NO" ]]; then
   echo "[SUCCESS] ${REP_DB_HOST} seems to be stopped and not replicating anymore."
 else
   echo "[ERROR] ${REP_DB_HOST} seems to be still connected to it's primary server!"
   exit 1
 fi
}
